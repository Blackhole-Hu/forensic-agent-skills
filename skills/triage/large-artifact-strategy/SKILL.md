...
- 发现加密、固件、XOR/key-like 或恶意载荷 Evidence → 只在 `route_candidates` 中分别记录 Pending `nas-raid-encrypted-storage`、`firmware-iot-forensics`、`proprietary-format-recovery` 或 `malware-forensics`
- 无可识别结构 → 继续 bounded sampling；只有产生重复边界、长度闭合、checksum、稳定字段或其他独立结构 Evidence 后才建议 `uncommon-media-triage`

Pending route candidate 必须包含 candidate skill、route basis、Artifact/Finding 引用、`high|medium|low` confidence、`current_availability: pending` 和 required next action。不得为 Pending Skill 创建 Route Step、Handoff、`visited_skills` 条目或已完成声明。
...
**Pending route candidates only**: proprietary-format-recovery, firmware-iot-forensics, nas-raid-encrypted-storage, malware-forensics
...
> **Note**: 所有 header candidate 为 false positive 或暂时无法识别常见结构时，不自动停止。保留 Artifact、bounded sample、负面 Finding 和排除路线；仍有批准范围内的结构测试时继续，没有新 Evidence 时交 `forensic-router` 或 `forensic-autopilot` 记录范围限制。不得直接调用 Pending Recovery Skill。
