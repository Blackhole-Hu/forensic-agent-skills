...
- 发现独立加密卷、通用 RAID、多成员存储或卷管理 Evidence → 只向 Router 提供 bounded regions、member candidates、结构 Evidence 和核心引用；无服务器/虚拟化关系时可记录 executable `nas-raid-encrypted-storage` candidate，LAS 不决定或直接调用该消费者
- 发现恶意载荷 Evidence → 只在 `route_candidates` 中记录 Pending `malware-forensics`
...
Pending malware candidate 必须包含 candidate skill、route basis、Artifact/Finding 引用、`high|medium|low` confidence、`current_availability: pending` 和 required next action。不得为 malware 创建 Route Step、Handoff、`visited_skills` 条目或已完成声明。
...
**Executable downstream selected by Router**: server-forensics-router, uncommon-media-triage, firmware-iot-forensics, nas-raid-encrypted-storage
**Pending route candidates only**: malware-forensics
...
> **Note**: 所有 header candidate 为 false positive 或暂时无法识别常见结构时，不自动停止。保留 Artifact、bounded sample、负面 Finding 和排除路线；仍有批准范围内的结构测试时继续，没有新 Evidence 时交 `forensic-router` 或 `forensic-autopilot` 记录范围限制。`large-artifact-strategy` 不得直接决定或调用 proprietary、firmware 或 storage，也不得直接调用 Pending Recovery Skill。
...
- 不得为 malware Pending Recovery Skill 创建 Route Step 或 Handoff
