...
uncommon-media-triage 位于 `file-triage`、按条件触发的 `large-artifact-strategy` 与 `forensic-router` 之后，负责对非常见、未知但显示重复结构的 Artifact 或 bounded candidate region 做只读结构识别。它验证记录边界、字段关系和反证，输出 `candidate_schema`、Finding 和 route candidates；当 Evidence 支持专有格式恢复时，它只生成 `proprietary-format-recovery` executable candidate 并返回 Router，不直接做恢复。它不做解密、固件解包、存储激活、恶意样本动态分析、数据库业务语义查询或最终答案生成。
...
| Server/Web/Database/Docker 证据 | 当前 Artifact/Finding 支持已实现消费者 | `forensic-router` re-evaluation | 必须产生新 Evidence；不得破坏现有服务器优先规则 | No |
...
| 固件容器/rootfs | 经验证的固件结构 Evidence | Pending `firmware-iot-forensics` route candidate | 单个 magic 或 weak hit 不足 | Candidate only |
| RAID/LVM/加密存储 | superblock/header/成员或层级 Evidence | Pending `nas-raid-encrypted-storage` route candidate | 有 PVE/Ceph/服务器关系时仍由 Server 路线优先 | Candidate only |
| PE/ELF/脚本/宏可疑载荷 | 有效格式加恶意分析目标或可疑上下文 | Pending `malware-forensics` route candidate | 普通固件程序且无样本分析 Evidence 不自动转出 | Candidate only |
...
`firmware-iot-forensics`、`nas-raid-encrypted-storage` 和 `malware-forensics` 固定使用 `current_availability: pending`。Pending candidate 不是 Route Step、Handoff 或已完成分析；不得加入 `visited_skills`，不得声称已调用对应 Skill。
...
本 Skill 不得直接调用 proprietary。它只输出满足上述字段要求的 executable route candidate，并通过下述 bounded re-entry 返回 Router。`firmware-iot-forensics`、`nas-raid-encrypted-storage` 和 `malware-forensics` 仍为 Pending，只存在于 `payload.route_candidates`，不得创建到它们的 Route Step 或 Handoff。
...
7. Router 重评时 `visited_skills` 已包含 `uncommon-media-triage`，因此不得再次选择 uncommon；输入完整且 proprietary candidate 合格时，Router 才可选择 `proprietary-format-recovery` 并加入 `visited_skills`。输入不足时只能在合法 hop/visited 范围内请求 `large-artifact-strategy` 补充 bounded Evidence，否则返回 `forensic-autopilot`；任何分支都不创建半完整 Handoff。

`forensic-router` → uncommon → Router → uncommon 的循环属于禁止路线。
...
- 已识别 proprietary executable candidate 或 Pending Recovery route candidate；停止越界深入，返回候选、Evidence 和范围限制，由 Router 决策。
...
- `proprietary-format-recovery` 已可执行，但只能由 Router 决定并调用；firmware、storage 和 malware 仍为 Pending。
