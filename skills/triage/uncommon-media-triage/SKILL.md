...
5. storage 和 malware 仍只记录 Pending candidate，不创建 Route Step、Handoff 或 `visited_skills` 条目。
6. PCAP、浏览器历史和完整移动设备取证写入 `excluded_routes`，并生成同时引用相关 Artifact 和本轮 Ledger Event 的 scope-limitation Finding。随后返回 `forensic-autopilot`；只有新 Evidence 需要重新判断消费者时，才按 Handoff 规则返回 `forensic-router`。
...
`nas-raid-encrypted-storage` 和 `malware-forensics` 固定使用 `current_availability: pending`。Pending candidate 不是 Route Step、Handoff 或已完成分析；不得加入 `visited_skills`，不得声称已调用对应 Skill。
...
本 Skill 不得直接调用 firmware，只输出 bounded regions、结构 Evidence、核心引用和 executable candidate，再通过同一次 bounded re-entry 返回 Router。`nas-raid-encrypted-storage` 和 `malware-forensics` 仍为 Pending，只存在于 `payload.route_candidates`。
...
7. Router 重评时 `visited_skills` 已包含 `uncommon-media-triage`，因此不得再次选择 uncommon；输入完整且 proprietary 或 firmware candidate 通过对应 input gate 时，Router 才可选择该消费者并加入 `visited_skills`。输入不足时只能在合法 hop/visited 范围内请求 `large-artifact-strategy` 补充 bounded Evidence，否则返回 `forensic-autopilot`；任何分支都不创建半完整 Handoff。

`forensic-router` → uncommon → Router → uncommon，以及 uncommon → Router → firmware → Router → uncommon 的循环属于禁止路线。
...
- 已识别 proprietary/firmware executable candidate 或 Pending storage/malware route candidate；停止越界深入，返回候选、Evidence 和范围限制，由 Router 决策。
...
- `proprietary-format-recovery` 和 `firmware-iot-forensics` 已可执行，但只能由 Router 决定并调用；storage 和 malware 仍为 Pending。
