...
6. malware 仍只记录 Pending candidate，不创建 Route Step、Handoff 或 `visited_skills` 条目。
...
`malware-forensics` 固定使用 `current_availability: pending`。Pending candidate 不是 Route Step、Handoff 或已完成分析；不得加入 `visited_skills`，不得声称已调用对应 Skill。
...
本 Skill 只形成独立存储 candidate 并返回 Router，不直接调用、组装或解锁。`malware-forensics` 仍为 Pending，只存在于 `payload.route_candidates`。
...
7. Router 重评时 `visited_skills` 已包含 `uncommon-media-triage`，因此不得再次选择 uncommon；输入完整且 proprietary、firmware 或 storage candidate 通过对应 input gate 时，Router 才可选择该消费者并加入 `visited_skills`。输入不足时只能在合法 hop/visited 范围内请求 `large-artifact-strategy` 补充 bounded Evidence，否则返回 `forensic-autopilot`；任何分支都不创建半完整 Handoff。
...
- 已识别 proprietary/firmware/storage executable candidate 或 Pending malware route candidate；停止越界深入，返回候选、Evidence 和范围限制，由 Router 决策。
...
- `proprietary-format-recovery`、`firmware-iot-forensics` 和 `nas-raid-encrypted-storage` 已可执行，但只能由 Router 决定并调用；malware 仍为 Pending。
