...
uncommon-media-triage 位于 `file-triage`、按条件触发的 `large-artifact-strategy` 与 `forensic-router` 之后，负责对非常见、未知但显示重复结构的 Artifact 或 bounded candidate region 做只读结构识别。它验证记录边界、字段关系和反证，输出 `candidate_schema`、Finding 和 route candidates；它不做解密、固件解包、存储激活、恶意样本动态分析、数据库业务语义查询或最终答案生成。
...
3. 发现 Recovery 证据时，只在 `payload.route_candidates` 记录 Pending 候选，不创建 Route Step 或 Handoff。
...
| XOR/key-like/known-plaintext 路线 | 可回指的变换或 key material Evidence | Pending `proprietary-format-recovery` route candidate | unknown 或高熵本身不足 | Candidate only |
...
四个 Pending Recovery route candidate 必须包含：
...
- `finding_refs`
- `confidence`
- `current_availability: pending`
- `required_next_action`

Pending candidate 不是 Route Step、Handoff 或已完成分析；不得加入 `visited_skills`，不得声称已调用对应 Skill。
...
本轮新增的唯一可执行领域目标是 `uncommon-media-triage`。四个 Pending Recovery Skill 只存在于 `payload.route_candidates`，不得创建到它们的 Route Step 或 Handoff。
...
7. Router 重评时 `visited_skills` 已包含 `uncommon-media-triage`，因此不得再次选择 uncommon；仍无兼容消费者时返回 `forensic-autopilot`，由 Answer Gate 校验范围限制并生成限制报告。
...
- 已识别 Pending Recovery route candidate；停止越界深入，返回候选和范围限制。
...
- 四个 Recovery Skill 仍为 Pending，直到各自迁移完成前不得创建实际 Handoff。
