...
3. 只有本轮产生新 Artifact 或 Finding，且 Handoff 章节全部条件满足时，才可最多一次返回 `forensic-router` 重评。
4. 本 Skill 不直接调用任何下游消费者，也不生成最终答案。
...
| 固件候选 | 经验证固件 header/container Evidence | Pending `firmware-iot-forensics` route candidate | `candidate_only` 或 `unknown` | Candidate only |
| RAID/LVM/加密卷候选 | 经验证成员、superblock、volume header 或层级 Evidence | Pending `nas-raid-encrypted-storage` route candidate | `candidate_only` 或 `unknown` | Candidate only |
| PE/ELF/脚本/宏或载荷候选 | 有效格式与可疑上下文 Evidence | Pending `malware-forensics` route candidate | `candidate_only` 或 `unknown` | Candidate only |
...
firmware、storage 和 malware 候选保持 Pending。对应 `route_candidates` 必须包含 `current_availability: pending`、candidate skill、Evidence 引用、`confidence` 和 `required_next_action`；不得为其创建 Route Step、Handoff 或 `visited_skills` 条目。
...
- firmware、storage 和 malware 保持 Pending，直到各自迁移完成。
