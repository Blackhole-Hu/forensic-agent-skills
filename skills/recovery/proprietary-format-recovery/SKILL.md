...
4. 只有本轮产生新 Artifact 或 Finding，且 Handoff 章节全部条件满足时，才可最多一次返回 `forensic-router` 重评。
5. 本 Skill 不直接调用任何下游消费者，也不生成最终答案。
...
storage 和 malware 候选保持 Pending。对应 `route_candidates` 必须包含 `current_availability: pending`、candidate skill、Evidence 引用、`confidence` 和 `required_next_action`；不得为其创建 Route Step、Handoff 或 `visited_skills` 条目。
...
- firmware 已可执行，但只能由 Router 决定；storage 和 malware 保持 Pending，直到各自迁移完成。
