...
description: 任务/材料路由器。根据检材类型、文件结构和证据线索，选择当前已实现的 Server、领域分析、uncommon media triage、proprietary format recovery 或 firmware static analysis 路径，或明确返回无兼容消费者。
---
...
| `request.payload` | Recommended | 上游提供的 uncommon、proprietary 或 firmware candidate regions、Evidence、候选 schema/layout 和限制；选择对应消费者前必须通过输入完整性检查 |
| `request.context` | Recommended | 当前 Route Record、step、Finding 和 Ledger Event 引用；选择 uncommon、proprietary 或 firmware 时必须包含当前 Route Record |
...
- 线索（配置文件、日志、凭据、服务器特征）
- uncommon、proprietary 或 firmware 候选对应的 `source_artifact_refs`、适用的 `candidate_regions`、结构/恢复 Evidence、`analysis_limits` 和 `extraction_limits`
...
`nas-raid-encrypted-storage` 和 `malware-forensics` 仍为 Pending。对应结果只能保留在 `payload.route_candidates`，并包含 `current_availability: pending`、candidate skill、Evidence 引用、`high|medium|low` confidence 和 `required_next_action`；本 Skill 不为它们创建 Route Step、Handoff 或 `visited_skills` 条目，也不描述为已调用或已完成。
...
将路由决策传递给 forensic-autopilot，由 autopilot 调用对应的专项 skill。选择 uncommon、proprietary 或 firmware 时同时传递原样 `request.payload` 和当前 Route Record context。
...
没有新 Evidence 时禁止 re-entry。Router 收到该 Handoff 时，`visited_skills` 已包含 `uncommon-media-triage`，因此不得再次选择 uncommon。只有 proprietary 或 firmware 对应 input gate 全部通过时才能选择该消费者并加入 `visited_skills`；输入不足时只能在合法 hop/visited 范围内请求 `large-artifact-strategy` 补充 bounded Evidence，否则返回 forensic-autopilot，且不创建半完整 Handoff。

proprietary 默认返回 autopilot。只有本轮产生新 Artifact 或 Finding 时，才允许最多一次 proprietary → Router re-entry，并且必须包含非空 `reentry_reason`、非空 `new_evidence_refs`、本轮新 Ledger Event，以及合法 `hop_count` 和 `routing_policy.max_hops`。Router 重评不得再次选择 `uncommon-media-triage` 或 `proprietary-format-recovery`；只有 proprietary 已生成经验证嵌套固件派生 Artifact 且 firmware input gate 全部通过时，才可选择 `firmware-iot-forensics` 并加入 `visited_skills`。没有其他当前可执行消费者时返回 autopilot。
...
明确禁止 `uncommon-media-triage` → Router → proprietary → Router → uncommon、proprietary → Router → proprietary、proprietary → Router → firmware → Router → proprietary、firmware → Router → firmware，以及 uncommon → Router → firmware → Router → uncommon。
...
**Downstream skills**: server-forensics-router, docker-container-forensics, database-server-forensics, webapp-server-forensics, uncommon-media-triage, proprietary-format-recovery, firmware-iot-forensics
...
- Router 是唯一消费者决策点；uncommon 只能提出 proprietary 或 firmware candidate，`large-artifact-strategy` 只提供 bounded regions、signature 和 Evidence，上游都不能直接调用消费者
- proprietary recovery 与 firmware analysis 必须分别通过完整 input、Route context、limits、Gate 和防循环检查
