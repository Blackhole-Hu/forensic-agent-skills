...
> **Note**: Unknown material type is not a stop condition by itself. Route unknown material to `uncommon-media-triage` or `proprietary-format-recovery` when candidate evidence supports that path.
