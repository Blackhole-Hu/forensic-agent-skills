...
本 skill 从 `competition-autopilot` 抽象而来，剥离了比赛专用逻辑（flag 格式、比赛计时器、队伍模式），保留通用调度框架。Competition 专项消费者属于 Pending Phase 4，当前不可调用。
...
- 判断检材应进入当前已实现的 Server/领域分析路径，或返回 `no-compatible-skill`
- 分配并行路径（如需要）
- `route_decision: no-compatible-skill` 表示当前仓库没有兼容消费者；它不是 Route Record、Route Step、Handoff 或 execution gate 的状态值
- Recovery 和 Competition 专项消费者尚未实现，不创建相关 Route Step 或 Handoff
