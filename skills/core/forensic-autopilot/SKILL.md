...
本 skill 保留通用取证调度框架。Phase 3 当前已实现 `uncommon-media-triage`、`proprietary-format-recovery` 与 `firmware-iot-forensics`；storage 和 malware 两个 Recovery Skill 仍为 Pending。
...
- 判断检材应进入当前已实现的 Server、领域分析、`uncommon-media-triage`、`proprietary-format-recovery` 或 `firmware-iot-forensics` 路径，或返回 `no-compatible-skill`
- 只有 Router 通过对应输入完整性检查并返回 uncommon、proprietary 或 firmware 决策时，才允许调用该 Skill
- 分配并行路径（如需要）
- `route_decision: no-compatible-skill` 表示当前仓库没有兼容消费者；它不是 Route Record、Route Step、Handoff 或 execution gate 的状态值
- `uncommon-media-triage`、`proprietary-format-recovery` 与 `firmware-iot-forensics` 是当前已实现的 Phase 3 消费者
- `nas-raid-encrypted-storage` 和 `malware-forensics` 仍未实现，不创建相关 Route Step 或 Handoff
...
- 调用 uncommon、proprietary 或 firmware 时，原样传递 Router 批准的 Request payload 和包含当前 Route Record 的 Route context；不得重新构造、删减、重命名或丢弃字段
- 结构、recovery-specific 与 firmware-specific 判断阈值只由 `forensic-router` 和对应领域 Skill 维护；autopilot 不重复判断、不生成或扩展候选，也不改变 analysis/extraction limits
...
5. 不得为仍为 Pending 的 `nas-raid-encrypted-storage` 或 `malware-forensics` 创建 Route Step、Handoff 或 `visited_skills` 条目。
...
**Passes to**: `tool-router` → `file-triage` → `large-artifact-strategy` (if triggered) → `forensic-router` → domain skills、`uncommon-media-triage`、`proprietary-format-recovery` 或 `firmware-iot-forensics` → `answer-gate` → `report-writer`
**Data available**: evidence-ledger 中的所有记录；调用 uncommon、proprietary 或 firmware 时还包括 Router 批准的原始 Request payload 和当前 Route Record context
...
- Phase 3 当前为部分实施状态；`uncommon-media-triage`、`proprietary-format-recovery` 和 `firmware-iot-forensics` 可执行
- storage 和 malware 两个 Recovery Skill 仍为 Pending，只能作为 route candidate，不得创建实际 Handoff
