...
本 skill 保留通用取证调度框架，不包含 flag 格式、比赛计时器或队伍模式等比赛专用逻辑。Phase 3 当前部分实施：`uncommon-media-triage` 可执行，四个 Recovery Skill 仍为 Pending；Competition 专项消费者属于 Pending Phase 4，当前不可调用。

## Use When

- 用户提供检材（文件、镜像、目录、远程入口）和目标（找什么、回答什么）
- 用户说"分析这个"、"看看这个镜像"、"帮我查一下"
- 比赛场景中提供题目附件和目标时，先使用当前通用取证链；没有已实现消费者的专项需求按 `no-compatible-skill` 处理
...
- 判断检材应进入当前已实现的 Server、领域分析或 `uncommon-media-triage` 路径，或返回 `no-compatible-skill`
- 只有 Router 通过 uncommon 输入完整性检查并返回 `uncommon-media-triage` 时，才允许调用该 Skill
- 分配并行路径（如需要）
- `route_decision: no-compatible-skill` 表示当前仓库没有兼容消费者；它不是 Route Record、Route Step、Handoff 或 execution gate 的状态值
- `uncommon-media-triage` 是当前唯一已实现的 Phase 3 消费者
- `proprietary-format-recovery`、`firmware-iot-forensics`、`nas-raid-encrypted-storage`、`malware-forensics` 和 Competition 消费者仍未实现，不创建相关 Route Step 或 Handoff
...
- 调用 uncommon 时，原样传递 Router 批准的 Request payload 和包含当前 Route Record 的 Route context；不得重新构造、删减、重命名或丢弃字段
- 具体结构阈值只由 `forensic-router` 和 `uncommon-media-triage` 维护，autopilot 不重复判断
...
**Passes to**: `tool-router` → `file-triage` → `large-artifact-strategy` (if triggered) → `forensic-router` → domain skills or `uncommon-media-triage` → `answer-gate` → `report-writer`
**Data available**: evidence-ledger 中的所有记录；调用 uncommon 时还包括 Router 批准的原始 Request payload 和当前 Route Record context
...
- Phase 3 当前为部分实施状态；只有 `uncommon-media-triage` 可执行
- 四个 Recovery Skill 仍为 Pending，只能作为 uncommon 输出中的 route candidate，不得创建实际 Handoff
- Competition 专项消费者尚未实现；当前没有兼容消费者时按 `no-compatible-skill` 记录证据和范围限制
