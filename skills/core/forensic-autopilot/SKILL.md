...
- 调用 `forensic-router` 判断检材类型和分析路径
- 调用 `tool-router` 选择执行环境（Windows/WSL/Docker/VM）
...
**Passes to**: `forensic-router` → `file-triage` → domain skills → `answer-gate` → `report-writer`
